...
class TestGetTrendingTopic:
    ...
    def test_returns_correct_schema(self, mock_ddg, mock_llm, mock_agent, mock_task, mock_crew_cls, mock_output):
        mock_ddg.return_value = [{"title": "LLM reasoning advances", "body": "", "href": ""}]
        mock_crew_cls.return_value.kickoff.return_value = self._make_crew_result("LLM reasoning and planning")
    ...
    @patch("agents.trend_agent.LLM")
    @patch("agents.trend_agent.ddg_search")
    def test_topic_is_stripped_of_quotes(self, mock_ddg, mock_llm, mock_agent, mock_task, mock_crew_cls, mock_output):
        mock_ddg.return_value = []
        mock_crew_cls.return_value.kickoff.return_value = self._make_crew_result('"Multimodal foundation models"')
        mock_output.parent.mkdir = MagicMock()
        mock_output.write_text = MagicMock()
    ...
    @patch("agents.trend_agent.LLM")
    @patch("agents.trend_agent.ddg_search")
    def test_sources_scanned_is_non_empty(self, mock_ddg, mock_llm, mock_agent, mock_task, mock_crew_cls, mock_output):
        mock_ddg.return_value = []
    ...
    @patch("agents.trend_agent.LLM")
    @patch("agents.trend_agent.ddg_search")
    def test_writes_output_file(self, mock_ddg, mock_llm, mock_agent, mock_task, mock_crew_cls, mock_output):
        mock_ddg.return_value = []
        mock_crew_cls.return_value.kickoff.return_value = self._make_crew_result("AI agents")
        mock_output.parent.mkdir = MagicMock()
        mock_output.write_text = MagicMock()
    ...
    def test_handles_no_search_results_gracefully(
        ...
        mock_crew_cls.return_value.kickoff.return_value = self._make_crew_result("retrieval augmented generation")
        mock_output.parent.mkdir = MagicMock()
        mock_output.write_text = MagicMock()
    ...
    @patch("agents.trend_agent.LLM")
    @patch("agents.trend_agent.ddg_search")
    def test_crew_kickoff_called_once(self, mock_ddg, mock_llm, mock_agent, mock_task, mock_crew_cls, mock_output):
        mock_ddg.return_value = []
